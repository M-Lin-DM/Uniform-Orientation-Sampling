...
I developed a signature of the point distribution that suggests the UOS algorithm finds a evenly-spaced set of points in several dimensions higher than 3. The code computing this signature can be found in `Evaluations.py`. See `demo.py` for usage. The signature is found by looking at the distribution of distances (measured in angular separation (radians)) from a point to its nearest neighbors. In the 3D case, points typically arrange in to an imperfect hexagonal lattice. This causes there to be a sharp spike in probability at a certain distance, namely, the distance of the first nearest neighbors. I found that this spike was present in dimensions up to 10, but I have not tested beyond that. In general, the distance distribution should appear multi-modal for evenly spaced points and unimodal/flat for projected points. Note: confusingly, the projection-based points can also be properly described as sampled "**uniformly at random**" on the sphere surface.
